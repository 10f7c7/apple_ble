...
void ams_notify_callback(SimpleBLE::ByteArray payload) {
    ...
    int data = (std::bitset<8>(ams_entity.EntityID).to_ulong() << 4) |
               std::bitset<8>(ams_entity.AttributeID).to_ulong();
...
void ancs_notify_callback(SimpleBLE::ByteArray payload) {
    ...
    ancs_notif_src.NotificationUID =
        (i[7] << 24) | (i[6] << 16) | (i[5] << 8) | i[4];

    std::bitset<32> x(ancs_notif_src.NotificationUID);

    std::cout << "EventID: " << ancs_notif_src.NotificationUIDvec[0] << ", "
              << ancs_notif_src.NotificationUIDvec[1] << ", "
              << ancs_notif_src.NotificationUIDvec[2] << ", "
              << ancs_notif_src.NotificationUIDvec[3] << std::endl;
...
void start_ams(SimpleBLE::Peripheral phone) {
    ...
    try {
        ...
        phone.write_request(AMS_UUID, AMS_ENTITY_UPDATE_UUID,
...
void CBLE::init() {
    ...
    std::cout << "Adapter identifier: " << adapter.identifier().value()
              << std::endl;
    ...
    phone.set_callback_on_disconnected([&]() {
        std::cout << "Disconnected from " << phone.identifier().value_or("UNKNOWN")
                  << " [" << phone.address().value_or("UNKNOWN") << "]"
                  << std::endl;
    ...
    while (!connect_was_successful) {
        std::cout << "Failed to connect to "
                  << phone.identifier().value_or("UNKNOWN") << " ["
    ...
    while (ble_async_thread_active) {
        std::this_thread::sleep_for(std::chrono::microseconds(100));
        while (!connect_was_successful) {
            while (!adapter.bluetooth_enabled().value()) {
                std::this_thread::sleep_for(std::chrono::seconds(2));
                std::cout << "sleeping cause no bluetooth" << std::endl;
            }
            std::cout << "Failed to connect to "
                      << phone.identifier().value_or("UNKNOWN") << " ["
                      << phone.address().value_or("UNKNOWN") << "]" << std::endl;
            std::this_thread::sleep_for(std::chrono::seconds(1));
            std::cout << "Retrying..." << std::endl;
            std::cout << "is bluetooth: " << adapter.bluetooth_enabled().value_or("IDK") << std::endl;
            connect_was_successful = phone.connect();
            if (connect_was_successful) {
                std::cout << "Starting AMS/-ANCS-" << std::endl;
                start_ams(phone);
...
int CBLE::disconnect() {
    ...
    // The connection will be destroyed when the last reference to it goes out of
    // scope.
...
int CBLE::sendCommand(int command) {
    connection.write_request(AMS_UUID, AMS_REMOTE_COMMAND_UUID,
                             {static_cast<char>(command)});
    return command;
}

int CBLE::sendNotification(ANCS_NOTIF_SRC_ATTR ancs_notif_src,
                           std::vector<uint8_t> actions) {

    SimpleBLE::ByteArray data = {
        0x00,
        static_cast<char>(ancs_notif_src.NotificationUIDvec[0]),
        static_cast<char>(ancs_notif_src.NotificationUIDvec[1]),
        static_cast<char>(ancs_notif_src.NotificationUIDvec[2]),
        static_cast<char>(ancs_notif_src.NotificationUIDvec[3]),
        (0x00),
        (0x01),
    ...
    //     std::cout << std::bitset<8>(static_cast<int>(i)).to_string() << " ";
    //     //to binary std::cout << std::hex << static_cast<int>(i) << " | ";
    // }
...
int CBLE::sendApplicationName(std::string application_name) {
    std::cout << "sendApplicationName: " << application_name << std::endl
              << std::endl;
    if (g_pANCSServer->application_index.find(application_name) !=
        g_pANCSServer->application_index.end()) {
        return 1;
    }
...
int CBLE::sendNotificationAction(std::vector<int> NotificationUID,
                                 int8_t action) {
    SimpleBLE::ByteArray data = {0x02,
                                 static_cast<char>(NotificationUID[0]),
                                 static_cast<char>(NotificationUID[1]),
                                 static_cast<char>(NotificationUID[2]),
                                 static_cast<char>(NotificationUID[3]),
                                 action};
